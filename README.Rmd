...
where the coefficient vector `b` has length K+1 (coefficients for x^K down to the constant term).


#### Notes

* If the degree K is even, the support is the full real line $(-\infty, \infty)$. 
* If K is odd, the support is the positive reals $(0, \infty)$. 
* The coefficient of the highest-order term (leading coefficient) must be negative.
...
Given the coefficients of the non-constant terms (for x^K, x^(K-1), ..., x), `NormalizingConstant` determines the log normalizing constant and returns the full coefficient vector including that constant. Coefficients are in descending order of power. 
