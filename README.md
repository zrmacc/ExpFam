...
[![R-CMD-check](https://github.com/zrmacc/ExpFam/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/zrmacc/ExpFam/actions/workflows/R-CMD-check.yaml)
...
`f(x) = exp(b[1]*x^K + b[2]*x^(K-1) + ... + b[K+1])`

where the coefficient vector `b` has length K+1 (coefficients for x^K
down to the constant term).

#### Notes

- If the degree K is even, the support is the full real line
  $(-\infty, \infty)$.
- If K is odd, the support is the positive reals $(0, \infty)$.
- The coefficient of the highest-order term (leading coefficient) must
  be negative.
- The constant term in `b` is the log of the normalizing constant.
...
Given the coefficients of the non-constant terms (for x^K, x^(K-1), …,
x), `NormalizingConstant` determines the log normalizing constant and
returns the full coefficient vector including that constant.
